...
## Варианты моделей для тестов

Для большей точности можно задать `OPENAI_STT_MODEL=gpt-4o-transcribe`. Модель `gpt-4o-transcribe-diarize` даёт разделение спикеров в одном потоке; полезна при двух каналах (user/other), но ответ API может отличаться по структуре — при использовании проверяйте актуальную документацию OpenAI.
