...
### Транскрипция микрофона: авто, браузер или сервер

В веб-приложении можно выбрать способ распознавания **микрофона** (канал «Вы»):

- **Авто** — браузерная STT, если доступна (Chrome, Edge), иначе серверная. Рекомендуется по умолчанию.
- **Браузер (Chrome)** — Web Speech API; аудио микрофона не отправляется на сервер. Нет галлюцинаций и лишних вызовов OpenAI для вашей речи.
- **Сервер** — микрофон отправляется на API, транскрипция через OpenAI (как канал «Собеседник»). Переключатель «Микрофон» отображается в панели управления при поддержке Web Speech API.
