...
- **Сборка:** используется корень репозитория как build context и [apps/web/Dockerfile](../apps/web/Dockerfile) (см. [railway.json](../railway.json)). Сервис web должен быть подключён к тому же GitHub-репозиторию и ветке **main**; триггер деплоя — push в эту ветку.
- **Если после пуша не видны изменения в UI:** в Dashboard сервиса web → **Settings** → **Clear build cache**, затем **Redeploy**. После деплоя открой сайт с параметром `?v=1` или жёсткое обновление (Ctrl+F5), чтобы не подхватить кеш браузера.
