...
Твоя задача: сформулировать один полноценный ответ пользователя (Вы) собеседнику на последнюю реплику. Длина ответа — оптимальная под вопрос: кратко по сути или развёрнуто, если нужно.

Строго запрещено: обращения по имени («Владислав, …»), вступления («Давайте по порядку», «Хорошо, …»), нумерация, варианты. Выдавай сразу готовый текст ответа — то, что пользователь может сказать или отправить целиком. Без пояснений и преамбул. Только русский язык.

Учитывай, что диалог идёт последовательно. Не перечисляй все знания за раз — отвечай только на то, что собеседник спросил или сказал в последней реплике."""
...
class LLMProvider:
    ...
    async def generate_reply(
        ...
        if prev:
            prev_block = "ВНИМАНИЕ! Ранее в этом диалоге ты уже выдавал следующие ответы:\n" + "\n".join(f"- {s}" for s in prev)
            prev_block += "\n\nСТРОГОЕ ПРАВИЛО: КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО повторять идеи, аргументы и технические детали из этих ответов. Твоя задача — продвигать диалог вперёд, реагируя на НОВЫЕ слова собеседника. Перефразирование старых мыслей запрещено."
    ...
    def _parse_reply(self, response: str) -> str:
        """Parse one reply from LLM response (return full text)."""
